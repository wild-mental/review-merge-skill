...
**하네스에 고정:** no-cascade는 한 번 어기면 preview 배포가 무더기로 발생하는 항구 규칙이므로 프로젝트 하네스에 박아 재발을 막는다 — Cursor는 `.cursor/rules/*.mdc`(또는 공통 `AGENTS.md`). 머지 후 정리(§5)·base 재타게팅처럼 정형화된 절차는 `.cursor/commands/`의 명령으로, push 직전 cascade 차단이 필요하면 `.cursor/hooks/`의 hook으로 승격한다.
...
- [ ] no-cascade 규칙을 하네스(`.cursor/rules/*.mdc`)에 고정(§6)
